...
type CompanyTabId = "overview_amount_by_company" | "overview_count_by_company";

type CompanyCharts = Partial<Record<CompanyTabId, SalesBudgetChartDataset | null>>;
...
const TABS: Array<{
  ...
  {
    id: "overview_amount_by_company",
    label: "Valor",
    icon: Coins,
    subtitle: "receita potencial",
  },
  {
    id: "overview_count_by_company",
    label: "Quantidade",
    icon: Building2,
    subtitle: "volume por filial",
  },
];

export default function SalesBudgetOverviewCompanyWidget({
  charts,
  isLoading,
  startDate,
  endDate,
  ...
  const [activeTab, setActiveTab] = useState<CompanyTabId>("overview_amount_by_company");
  ...
  const title = useMemo(
    () => activeChart?.title ?? "Empresa / filial",
    [activeChart?.title]
  );
    ...
    <>
      <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-neutral-100 px-4 py-3">
          <div>
            <h3 className="text-base font-black tracking-tight text-neutral-900">Empresa / filial</h3>
            <p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-neutral-500">
              compare concentração por unidade
            </p>
          </div>
