...
export default function EfficiencyKpiCards({ data, isLoading }: EfficiencyKpiCardsProps) {
    ...
    const kpis = [
        ...
        { label: "Atraso Rec. (Médio)", value: `${data.diasMedioAtrasoReceber.toFixed(0)}d`, icon: Calendar, color: "text-rose-600", bg: "bg-rose-50", sub: "Dias em atraso" },
        ...
        { label: "Prazo Rest. Pag.", value: `${data.prazoMedioRestantePagar.toFixed(0)}d`, icon: Clock, color: "text-rose-600", bg: "bg-rose-50", sub: "Média p/ vencer" },
        { label: "Parcelas em Atraso", value: formatPercent(data.percParcelasAtraso), icon: AlertTriangle, color: "text-red-600", bg: "bg-red-50", sub: "Risco Imediato" },
        ...
        { label: "Cresc. Pagar", value: formatPercent(data.crescimentoPagamentos), icon: TrendingUp, color: "text-rose-600", bg: "bg-rose-50", sub: "Evolução Custos" },
        ...
        { label: "% Pag. Antecip.", value: formatPercent(data.percPagamentoAntecipado), icon: Zap, color: "text-rose-500", bg: "bg-rose-50", sub: "Pagto antecipado" },
    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {kpis.map((kpi, idx) => (
                <div key={idx} className="bg-white p-3 rounded-xl border border-neutral-100 shadow-sm hover:shadow-md transition-shadow">
