...
import { Bot, User, X, Send, Loader2, ArrowLeft, Info, MessageSquare, Sparkles, Calendar, Database, ChevronDown, ChevronUp, RefreshCw } from "lucide-react";
...
export default function ChartAnalysisView({ id, title, description: propDescription, chartComponent, data, onClose, initialStartDate, initialEndDate, onDateChange }: ChartAnalysisViewProps) {
    ...
    useEffect(() => {
        // Try to load saved messages from the database
        const loadSavedMessages = async () => {
            if (!userId) {
                // Session not ready yet, set default greeting
                setMessages([{
                    id: "initial",
                    role: "assistant",
            ...
            try {
                const sessionId = `chart-${id}-${userId}`;
                ...
                if (res.data && res.data.length > 0) {
                    ...
                    setHasAsked(true); // Hide suggested questions if there's history
                } else {
                    setMessages([{
                        id: "initial",
                        role: "assistant",
                ...
                // If session doesn't exist yet, show default greeting
                setMessages([{
                    id: "initial",
                    role: "assistant",
                    content: `Olá! Estou analisando os dados de **${title}**. \n\n${description}\n\nO que você gostaria de entender especificamente sobre estes números?`
                ...
                <div className="w-full lg:w-[460px] bg-white flex flex-col shrink-0 shadow-[-10px_0_30px_-15px_rgba(0,0,0,0.05)] relative">
                    <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-black text-neutral-900 uppercase tracking-widest">Conversa com Especialista</span>
                        </div>
                        {isTyping && (
                    ...
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-neutral-100">
                        {messages.map((msg) => (
